...
def load_batched_data(specPath, targetPath, batchSize):
    ...
    return data_lists_to_batches([np.load(fn) for fn in glob.glob(os.path.join(specPath, '*.npy'))],
                                 [np.load(fn) for fn in glob.glob(os.path.join(targetPath, '*.npy'))],
                                 batchSize) + \
            (len(glob.glob(os.path.join(specPath, '*.npy'))),)